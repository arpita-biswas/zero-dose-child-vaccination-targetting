...
4. Feature_importance.ipynb: Creates a graph showing the importance of each predictor when trained on a particular country-year using a prediction algorithm.
5. Comparison_geotarget_vs_model.ipynb: Compares the performance of the machine learning approach to a simple geographic rule that target districts with less than 80% full-immunization coverage according to the survey data. 
